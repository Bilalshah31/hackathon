...
firebase.auth().onAuthStateChanged((user)=>{
    ...
    if(user){
        ...
        firebase.database().ref(`users/${uid}`).once('value', (data) => {
            ...
            // let profileCountry = document.getElementById("profile-country")
...
let saveChanges = () => {
    ...
    let profileAdress = document.getElementById("profile-address");

    localStorage.setItem("date",profileDob.value)
    console.log(profileUsername.value)
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
            firebase.database().ref(`users/${user.uid}`).update({username : profileUsername.value , dob : profileDob.value , phone : profilePhone.value , address:profileAdress.value, })
            .then(()=>{
